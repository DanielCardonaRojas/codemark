...
pub async fn handle_search(cli: &Cli, mode: &OutputMode, args: &SearchArgs) -> Result<()> {
    ...
    if dbs.len() == 1 {
        ...
        let annotated: Vec<output::AnnotatedBookmark> = all
            .iter()
            .map(|(idx, bm)| output::AnnotatedBookmark { source: &dbs[*idx].0, bookmark: bm })
            .collect();
        ...
        if needs_line {
            ...
            let get_line_fn = |_label: &str, full_id: &str| -> Option<usize> {
                line_cache.get(&(_label.to_string(), full_id.to_string())).copied()
            };
...
async fn handle_semantic_search(
    ...
    if matches!(mode, OutputMode::Json) {
        ...
        let annotated: Vec<output::AnnotatedBookmark> = bookmarks
            .iter()
            .map(|(_, idx, bm)| output::AnnotatedBookmark { source: &dbs[*idx].0, bookmark: bm })
            .collect();
...
async fn handle_collection_search(
    ...
    let results: Vec<(Option<String>, codemark_core::engine::bookmark::Collection, usize)> =
        if args.semantic {
            ...
            let mut out: Vec<(Option<String>, codemark_core::engine::bookmark::Collection, usize)> =
                Vec::new();
            let mut any_ok = false;
            let mut last_err: Option<Error> = None;
