...
pub fn resolve_bookmark_file_path(file_path: &str, db_path: &Path) -> Result<PathBuf> {
    ...
    let base_path = if let Some(root) = repo_root_from_db {
        ...
        if let Some(ctx) = detect_context(db_path.parent().unwrap_or(db_path)) {
            ctx.repo_root
        } else {
    ...
    // Ensure base_path is absolute; if not, convert it using current_dir
    let base_path = if base_path.is_absolute() {
        base_path
    } else {
        std::env::current_dir()?.join(&base_path).canonicalize().unwrap_or(base_path)
    };
